...
def synthesize_logistics(df):
    ...
    # 2. Traffic Level (Low, Medium, High, Jam)
    # Higher distance slightly increases chance of worse traffic
    traffic_levels = []
    for d in df["distance_km"]:
        if d > 10:
            traffic_levels.append(np.random.choice(["Medium", "High", "Jam"], p=[0.2, 0.5, 0.3]))
        elif d > 5:
            traffic_levels.append(np.random.choice(["Low", "Medium", "High", "Jam"], p=[0.1, 0.4, 0.4, 0.1]))
        else:
            traffic_levels.append(np.random.choice(["Low", "Medium", "High"], p=[0.5, 0.3, 0.2]))
    df["traffic_level"] = traffic_levels
    
    # 3. Food Preparation Time (10 to 45 mins)
    df["food_preparation_time"] = np.random.randint(10, 45, n)
    
    # 4. Actual Delivery Time (depends on distance and traffic)
    ...
    # 5. Expected Delivery Time (Simulated Swiggy ETA)
    ...
    df["time_of_day"] = pd.cut(df["order_hour"], bins=[0, 11, 16, 20, 24], labels=["Morning", "Afternoon", "Evening", "Night"], right=False)
